...
def get_text_config(config: AutoConfig) -> AutoConfig:
    ...
    VLM models (e.g. Qwen3_5, InternVL) have a top-level config with text_config
    and vision_config sub-configs. LoRA operates on the text model, so we need
    attributes like hidden_size, num_hidden_layers etc. from text_config.
